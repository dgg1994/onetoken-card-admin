<template>
  <div class="app-container">
          ...
          >
            ...
            <el-form-item label="" prop="tel">
              <el-input
                ...
                style="width: 240px"
                @keyup.enter.native="handleQuery"
                @input="queryParams.tel = queryParams.tel.replace(/\D/g, '')"
              />
            </el-form-item> -->
    ...
    <el-dialog :title="title" :visible.sync="open" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="auto">
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="渠道商等级" prop="levelNo">
              <el-select
                v-model="form.levelNo"
                size="medium"
                collapse-tags
                placeholder="请选择渠道商等级"
              >
                <el-option
          ...
          <el-col :span="12">
            <el-form-item label="密码" prop="password" v-if="!form.id" porp="password">
              <el-input v-model="form.password" placeholder="请输入密码" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="IP 地址">
              <el-input v-model="form.ipWhite" placeholder="请输入IP地址" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="谷歌私钥" prop="googleSecretkey">
              <el-input
                v-model="form.googleSecretkey"
                maxlength="11"
                readonly
              />
            </el-form-item>
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="resetUserPwd" v-debounce
          >确 定</el-button
        >
...
<script>
import { getPasswordOrKey } from "@/api/system/user";
import {
  channelUserFindAll,
  channelUserAdd,
  channelUserUpdate,
  channelUserDelete,
  channelUserUpdatePwd,
  findChannelLevelList,
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        levelNo: [{ required: true, message: "等级不能为空", trigger: "blur" }],
        tel: [
          {
            required: true,
            message: "请输入手机号码",
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      channelUserFindAll(this.queryParams).then((response) => {
      ...
      this.$refs["form"].validate((valid) => {
        if (valid) {
          ...
          if (this.form.id != undefined) {
            channelUserUpdate(this.form).then((response) => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            channelUserAdd(this.form).then((response) => {
    ...
    handleDelete(row) {
      this.$modal
        .confirm("是否确认删除用户" + row.acctive + "？")
        .then(function () {
          return channelUserDelete({ userId: row.userId });
        })
        .then(() => {
