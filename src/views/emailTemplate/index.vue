      ...
      <div class="conetntBox">
        <div class="flex_sb">
          <el-form :model="queryParams" size="small" :inline="true" label-width="68px">
           ...
           <el-form-item label="" prop="templateNumber">
              <el-input v-model="queryParams.templateNumber" placeholder="请输入消息编号" clearable style="width: 240px"
        ...
        <el-dialog :title="title" :visible.sync="dialogOpen" width="60%" :close-on-click-modal="false" append-to-body>
          <el-form ref="formData" :model="formData" :rules="rules" label-width="100px">
            ...
            <el-row>
              <el-col :span="24">
                <el-form-item label="模板编号" prop="templateNumber">
                   <el-input type="number" v-model="formData.templateNumber" placeholder="请输入模板编号" :disabled="upState"/>
                </el-form-item>
              </el-col>
            </el-row>
            ...
            <el-row>
              <editor v-model="formData.templateContent":min-height="340"/>
            </el-row>
...
import { emailTemplateList,emailTemplateAdd,emailTemplateUpdate,emailTemplateDelete} from "@/api/template/template";
export default {
  name: "typesOfPoints",
  components: {
  ...
  methods: {
    ...
    getList() {
      ...
      emailTemplateList(this.queryParams).then(res => {
      ...
      this.$refs["formData"].validate(valid => {
        if (valid) {
          if(this.formData.id){//编辑
             emailTemplateUpdate(this.formData).then(res => {
              if(res.code == 200){
            ...
            emailTemplateAdd(this.formData).then(res => {
              if(res.code == 200){
    ...
    handleDelete(row) {
      this.$modal.confirm('是否确认删除？').then(function () {
        return emailTemplateDelete(row.id);
      }).then(() => {
        this.getList();
